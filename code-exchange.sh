...
if [[ -n "${kid}" && -n "${private_pem}" && -f "${private_pem}" ]]; then
  readonly assertion=$(./jwt/client-assertion.sh -a "${issuer}" -i "${AUTH0_CLIENT_ID}" -k "${kid}" -f "${private_pem}" -A "${alg}" )
...
if [[ -n "${dpop_pem_file}" ]]; then
    dpop_header="DPoP: $(./jwt/dpop.sh -r "${dpop_pem_file}" -m POST -u "${token_endpoint}")"
...
if [[ -n "${dpop_pem_file}" ]]; then
  ...
  if [[ -n "${_dpop_nonce}" ]]; then
    dpop_header="DPoP: $(./jwt/dpop.sh -r "${dpop_pem_file}" -m POST -u "${token_endpoint}" -n "${_dpop_nonce}")"
    [[ -n "${opt_verbose}" ]] && echo "${dpop_header}"
    curl -s --request POST \
      -H "${authorization_header}" \
      -H "${dpop_header}" \
